...
model_1_3 <- 
  ...
  scale_y_discrete(
    labels = c(nback_dprime_centered = "2-back d'",
               ...
               event_unpleasantness_centered = "Perceived distress",
...
(set_model <- 
   model_1_3 + model_5_7 + model_2_4 + model_6_8 &
   plot_layout(guides = 'collect') &
...
ggplot(data = df, aes(x1, y1, color = react)) +
  ...
  geom_smooth(method = "lm", size = 1.5, alpha = 1, se = F) +
 # theme_void() +
  ...
  labs(x = "Perceived distress", y = "Negative affect", title = "", color = "Emotional\nreactivity:") +
